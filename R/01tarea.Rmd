...
plot_residuals <- function(fit, geom.size = 2, remove.estimates = NULL, show.lines = TRUE,
  ...
  # residual plot
  res.plot <- res.plot +
    facet_wrap(~grp, nrow= 3, scales = "free") +
...
- Interpretar estimación puntual de b1 y b2 y luego significancia estadística y economica 
```{r est-model}
...
Uno de los supuestos importantes de *OLS*[^1] es que el valor esperado de los residuos es cero ($E(\widehat{u})=0$), o en términos matriciales que la suma de los residuos $\widehat{u}_i$ es igual a cero [^2]

$$
E(u) = \sum\limits_{i=0}^n{\widehat{u}_i} = `r round(sum(modeldata$residuals),3)`
$$
...
modeldata %>%
  ...
  filter(name != "growth") %>% 
  kableExtra::kable(., col.names = c("Variables independientes ($X$)", "$E(u|X_i)$")) 

```
Como modo de complementar este análisis, una forma convencional es la visualización de los residuos, dado ciertos parámetros de los modelos [^3]. Como se puede ver en la figura ...(aquí describir)


```{r orthogonal, results='asis', fig.cap="Distribución de residuos para cada predictor del modelo"}
...
Por ley de esperanzas iteradas decimos que  $E(y_i) = E[E(y_i|X)]$, donde $E(y_i|X)$ representa el valor esperado de *y* que ha sido estimado a partir de un cambio en $X$, manteniendo lo demas constante (en otras palabras $\widehat{y}$ estimado). 

Luego, calculamos la media de $\widehat{y}$ en los datos del modelo, obteniendo `r round(mean(modeldata$.fitted),5)`. Mientras que, tal como reportamos en el *Cuadro 1* la media de la tasa de crecimiento en la muestra es `r round(mean(data$growth),5)`
...
Según Wooldrige (2019) $R^2$ se puede mostrar como el cuadrado del coeficiente de coorelaci´´on entre las $y_i$ reales y los valores ajustados de $\widehat{y}_i$. Formalmente

$$
R^2 = \frac{(\sum\limits_{i=0}^n{(y_i - \widehat{y})(\widehat{y}_i - \bar{\widehat{y}})})^2}{{\sum\limits_{i=0}^n{(y_i - \widehat{y})^2}}\cdot{(\widehat{y}_i - \bar{\widehat{y}})}^2}}
$$
